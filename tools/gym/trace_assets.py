...
the game actually reached for.

Three things make a naive trace wrong, and all are handled here:

* Opening is not the only way the game touches a file. A material names its
  textures, and the game finds each one by stat-ing candidate directories until
  one answers; it opens only the textures it goes on to draw. Watching openat
  alone recorded 853 files for a track where the stat family records 1714, and
  the 861 it missed were real: the pack built from them drew hacienda with 284
  textures absent, complaining about every one and rendering anyway. So the
  trace follows newfstatat, statx and access as well.
...
Verify the result with tools/gym/verify_pack.py, which both reads the game's
log for assets it could not find and compares frames against the full asset
set. Both are needed: a wholly untextured pack shows up in the frames, while a
few absent detail maps do not move the pixels enough to notice and are only
visible as complaints in the log.
...
# Every way the game can ask whether a file is there. Opening is the obvious
# one and the incomplete one: texture lookup stats its candidates and opens only
# what it draws, so openat alone misses about half of what a pack needs.
_SYSCALLS = "openat,newfstatat,statx,access"
...
def trace_one(track: str, race_py: Path, assets: Path, tmp: Path) -> set[str]:
    """Race `track` under strace and return the stk-assets paths it reached for."""
    out = tmp / f"trace_{track}.txt"
    # Cold cache, or the trace silently under-reports by about 40%.
    shutil.rmtree(cache_dir(), ignore_errors=True)
    subprocess.run(
        ["strace", "-f", "-qq", "-e", "trace=" + _SYSCALLS, "-o", str(out),
    ...
    for line in out.read_text(errors="replace").splitlines():
        # A failed lookup says nothing about what the pack needs: the game
        # stats several candidate directories per texture and all but one of
        # them miss.
...
def main() -> int:
    ...
    header = [
        ...
        "#",
        "# Generated by tools/gym/trace_assets.py: every file the game opened or",
        "# stat-ed while racing each track below, with a cold texture cache. Both",
        "# a warm cache and watching opens alone hide about half of this list, so",
        "# regenerate it with that script, not by hand.",
        "#",
